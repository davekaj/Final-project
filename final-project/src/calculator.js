...
class Calculator extends React.Component{
	constructor(){
		super();
		this.state = { singleCrypto:  //same as the seed, just a temporary holder. MIGHT BE ABLE TO REMOVE THIS AND HAVE BLANK*****
																[/*{
																	text: 'Bitcoin', 
																	price: 0.00, 
																	amount: 0, 
  ...
  componentWillMount(){
	...
	addCryptoLine(newItem){
		...
		let newObjectList = {
			...
			amount: 0,
			originalPrice: 0.00,
			id: (copyOfList.length === 0 ? Number(1) : Number(copyOfList[copyOfList.length-1].id +1)) //ternary operator because if the list is 0 long it breaks. need to give id 1 for a 0 length list
		};
...
class RenderCryptoList extends React.Component {
	...
	this.state = {
		updateAmountState: {/*text: 'Test', price: 0.00, originalPrice: 0.00, amount: 0, id: 1*/}, // is this needed? i have never seen it printed
	}
}
...
//updates the state of the child component so it is up to date when the form will be submitted
  onInputList(e) {
    this.setState({
      ...
      updateAmountState: 	{text: e.target.name, price: e.target.title, originalPrice: e.target.className, amount: e.target.value, id: e.target.id},
    });
  }
				...
				<form onSubmit={(e)=>{this.onSubmitList(e,`clearAmount${i}`)}} key={i}>
					<li className="customList col-xs-12" >
						<span className="col-xs-12 col-sm-5 col-md-5 col-lg-5" id="listBorders">
							...
							<div>
								<input type="text" style={{width:'200px'}} placeholder="insert amount you own"  onChange={ this.onInputList } name={list[i].text} id={list[i].id} title={list[i].price} ref={`clearAmount${i}`} className={list[i].originalPrice}></input> 
									<span> </span>
